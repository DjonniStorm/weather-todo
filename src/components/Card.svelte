<script lang="ts">
    ...
    onMount(async () => {
        ...
        city = addres.error
            ? addres.error
            : (addres.addres!.city ?? addres.addres!.state);
        if (addres.addres) {
    ...
    const getNeededWeather = (weathr: WeatherResponse<WeatherData>) => {
        ...
        const closestIndex = weathr.time.reduce(
            (closestIdx, currentTime, index) => {
                const currentDiff = Math.abs(
                    currentTime.getTime() - now.getTime(),
                );
                const closestDiff = Math.abs(
                    weathr.time[closestIdx].getTime() - now.getTime(),
                );
        ...
        const result: WeatherData[] = weathr.time
...
<div
    class="rounded-4xl flex flex-col justify-center items-center gap-10 border-amber-300 border-2 shadow-2xl p-10 overflow-hidden md:max-w-10/12"
>
    <section class="">
        <h1>{addres_state}</h1>
